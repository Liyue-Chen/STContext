...
from UCTB.model.MTGNN_context_st import gtnet
import pdb
import time
...
context_args = {'name':name,'type':'AQI','is_multicolumn':args.is_multicolumn}
...
lat_lng_list = np.array([[float(e1) for e1 in e[2:4]]
                                     for e in node_traffic_loader.dataset.node_station_info])

new_latlng_array = lat_lng_list[node_traffic_loader.traffic_data_index]
for s in range(new_latlng_array.shape[0]):
    expected_station_info.append((new_latlng_array[s,0],new_latlng_array[s,1]))


expected_time_range = node_traffic_loader.dataset.time_range
expected_time_fitness = node_traffic_loader.dataset.time_fitness
stcontextloader.TTransformation(expected_time_fitness=expected_time_fitness,expected_time_range=expected_time_range)
assignment_matrix = stcontextloader.get_assignment_matrix(expected_station_info)
stcontextloader.st_context_transformed = np.transpose(np.array(list(stcontextloader.latlon2context.values())),(1,0,2))

stcontextloader.get_feature(args.context_historical_window,node_traffic_loader.train_index,node_traffic_loader.test_index,future_window=args.context_future_window)
...
args.seq_out_len = 1
args.save = os.path.abspath('./experiment/MTGNN_{}_{}_{}_{}/'.format(args.dataset, args.city, args.MergeIndex,args.seed))
context_window_size = args.context_historical_window+args.context_future_window+1 # 1 for current time slot
# pdb.set_trace()
...
data_dict = load_dataset(node_traffic_loader, args.batch_size, args.batch_size, args.batch_size,st_context_loader=stcontextloader)
# 需要改下
...
engine = Trainer_context(model, args.learning_rate, args.weight_decay, args.clip, args.step_size1, args.seq_out_len, device, False)
outputs = []
realy = torch.Tensor(data_dict['y_test']).to(device)
realy = realy.transpose(1, 3)[:, 0, :, :]
am = torch.Tensor(assignment_matrix).to(device)
for iter, (x,context, y) in enumerate(data_dict['test_loader'].get_iterator()):
...
for i in range(1,args.epochs+1):
    ...
    #     engine.model.enter_mode('finetune')
    for iter, (x,context, y) in enumerate(data_dict['train_loader'].get_iterator()):
        # pdb.set_trace()
        ...
        if iter%args.step_size2==0:
            ...
            for j in range(args.num_split):
                ...
                metrics = engine.train((tx,tc,am), ty[:,0,:,:],id)
    ...
    valid_rmse = []
    s1 = time.time()
    for iter, (x,context, y) in enumerate(data_dict['val_loader'].get_iterator()):
        testx = torch.Tensor(x).to(device)
        testx = testx.transpose(1, 3)
        testc = torch.Tensor(context).to(device)
        testc = testc.transpose(1, 3)
        ...
        if i <= args.pretrain_epochs:
            ...
            metrics = engine.eval((testx,testc,am), testy[:,0,:,:])
...
for iter, (x,context, y) in enumerate(data_dict['test_loader'].get_iterator()):
    ...
    with torch.no_grad():
        if args.epochs <= args.pretrain_epochs:
            ...
            preds = engine.model(testx,testc,am)
            preds = preds.transpose(1, 3)
    outputs.append(preds.squeeze())
y_truth = node_traffic_loader.normalizer.inverse_transform(node_traffic_loader.test_y)
yhat = torch.cat(outputs, dim=0).cpu().numpy()
