...
graph_obj = GraphGenerator(graph='distance', data_loader=uctb_data_loader,threshold_distance=1000)
...
model = gtnet(args.gcn_true, args.buildA_true, args.gcn_depth, args.num_nodes,
                  ...
                  seq_length=args.seq_in_len,predefined_A=graph_obj.AM[0], in_dim=args.in_dim, out_dim=args.seq_out_len,
...
for iter, (x, y) in enumerate(data_dict['test_loader'].get_iterator()):
    ...
    with torch.no_grad():
        preds = engine.model(testx)
        preds = preds.transpose(1, 3)
        # print(iter,torch.mean(testx),torch.mean(preds))
    outputs.append(preds.squeeze())
