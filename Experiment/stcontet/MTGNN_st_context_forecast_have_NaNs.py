...
for i in range(uctb_data_loader.train_y.shape[0]):
    for j in range(uctb_data_loader.train_y.shape[1]):
        # pdb.set_trace()
        closeness = uctb_data_loader.train_closeness[i,j,:,0]
        closeness[np.nonzero(closeness<0)] = 0
        period = uctb_data_loader.train_period[i,j,:,0]
        period[np.nonzero(period<0)] = 0
        trend = uctb_data_loader.train_trend[i,j,:,0]
        trend[np.nonzero(trend<0)] = 0
        assert np.count_nonzero(uctb_data_loader.train_closeness[i,j,:,:]<0) == 0
        assert np.count_nonzero(uctb_data_loader.train_period[i,j,:,:]<0) == 0
        assert np.count_nonzero(uctb_data_loader.train_trend[i,j,:,:]<0) == 0
        if uctb_data_loader.train_y[i,j,0]<0:
            if uctb_data_loader.train_y[i-1,j,0] >= 0:
                uctb_data_loader.train_y[i,j,0] = uctb_data_loader.train_y[i-1,j,0]
            elif uctb_data_loader.train_y[i-uctb_data_loader.daily_slots,j,0] >= 0:
                uctb_data_loader.train_y[i,j,0] = uctb_data_loader.train_y[i-uctb_data_loader.daily_slots,j,0]
            else:
                print('fuck')
for i in range(uctb_data_loader.test_y.shape[0]):
    for j in range(uctb_data_loader.test_y.shape[1]):
        closeness = uctb_data_loader.test_closeness[i,j,:,0]
        closeness[np.nonzero(closeness<0)] = 0
        period = uctb_data_loader.test_period[i,j,:,0]
        period[np.nonzero(period<0)] = 0
        trend = uctb_data_loader.test_trend[i,j,:,0]
        trend[np.nonzero(trend<0)] = 0
        assert np.count_nonzero(uctb_data_loader.test_closeness[i,j,:,:]<0) == 0
        assert np.count_nonzero(uctb_data_loader.test_period[i,j,:,:]<0) == 0
        assert np.count_nonzero(uctb_data_loader.test_trend[i,j,:,:]<0) == 0
        if uctb_data_loader.test_y[i,j,0]<0:
            uctb_data_loader.test_y[i,j,0] = 0
