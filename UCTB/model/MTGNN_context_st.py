...
class gtnet(nn.Module):
    def __init__(self, 
        ...
        self.channel_modeling_1 = nn.Linear(in_features=context_dimensions,out_features=context_hidden_size,bias=False)
    ...
    def enter_mode(self,mode):
            ...
            self.temporal_modeling.requires_grad_(False if mode =='pretrain' else True)
            if self.spatial_modeling_method=='mlps':
                self.spatial_modeling.requires_grad_(False if mode =='pretrain' else True)
            self.channel_modeling_1.requires_grad_(False if mode =='pretrain' else True)
            self.gate_linear.requires_grad_(False if mode =='pretrain' else True)
    def forward(self, input, st_context=None, assignment_matrix=None, idx=None):
        # print('in')
        # pdb.set_trace()
